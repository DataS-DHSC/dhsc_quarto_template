...
html_sidebar2 <- function(selected, names, area, subnames = NULL) {
  html <- '<html><link rel="stylesheet" href="./www/GDS.css"><br><br><ul class="app-subnav__section">'
  ...
  for (name in names) {
    ...
    link <- paste0('<li class="', class, '"><a class="app-subnav__link govuk-link govuk-link--no-visited-state govuk-link--no-underline" href="', Sys.getenv('BASE_URL'), area, '/', name, '.html">', name1, end)
    ...
    if ((selected == name) & !(is.null(subnames))) {
      ...
      for (chart in subnames) {
        ...
        link <- paste0(subclass, Sys.getenv('BASE_URL'), area, '/', name, '.html#', chart2, '"> ', chart, '</a></li>')
